...
/**
 * A day's entry in the manifest: its track and Trips files, and when its first Train comes onto the map and its last
...
export function manifestDay(bundle: Bundle, files: Pick<ManifestDay, 'track' | 'trips'>): ManifestDay {
  ...
  return { date: bundle.serviceDay, ...files, from: bundle.noonMinus12h + from * 1000, to: bundle.noonMinus12h + to * 1000 };
}
